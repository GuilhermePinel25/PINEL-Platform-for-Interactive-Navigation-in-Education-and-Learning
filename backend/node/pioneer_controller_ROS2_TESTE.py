...
odometry_log = []
start_time = None
last_twist = {'linear_x': 0.0, 'angular_z': 0.0}
...
class PioneerController(Node):
    ...
    def mover_para_frente(self, distancia):
        ...
        while erro > 0.01:
            ...
            last_twist['linear_x'] = float(twist.linear.x)
            last_twist['angular_z'] = float(twist.angular.z)
            # rate.sleep()
    ...
    def girar(self, angulo):
        ...
        ganho = 2.5  # quanto maior, mais agressivo o giro, manter 1.5
...
def classify_stream():
    def generate_frames():
        ...
        model = YOLO("D:\\PINEL\\blockly-ROS-project\\Gesture_Recognition\\libras\\runs\\detect\\train\\weights\\best.pt")
        seguir = True
        detected_class = None
...
@app.route('/bridge/cmd_vel', methods=['GET'])
def bridge_cmd_vel():
    """Retorna a última velocidade publicada (para o simulador MATLAB Online)."""
    return jsonify({
        'linear_x': last_twist['linear_x'],
        'angular_z': last_twist['angular_z'],
        'ts': time.time()
    })

@app.route('/bridge/pose', methods=['POST'])
def bridge_pose():
    """Recebe a pose integrada pelo MATLAB e atualiza o estado interno."""
    global posicao_atual, orientacao_atual, odometry_log
    data = request.get_json(force=True)
    x = float(data.get('x', posicao_atual['x']))
    y = float(data.get('y', posicao_atual['y']))
    yaw = float(data.get('yaw', orientacao_atual))

    posicao_atual['x'] = x
    posicao_atual['y'] = y
    orientacao_atual = yaw

    # Log simples (x, y, z=0, yaw, t)
    odometry_log.append([x, y, 0.0, yaw, time.time()])
    return jsonify({'ok': True})

@app.route('/bridge/run_start', methods=['POST'])
def bridge_run_start():
    global execution_active
    execution_active = True
    return jsonify({'ok': True})

@app.route('/bridge/run_end', methods=['POST'])
def bridge_run_end():
    global execution_active
    execution_active = False
    return jsonify({'ok': True})
...
def bridge_status():
    return jsonify({'active': execution_active})
